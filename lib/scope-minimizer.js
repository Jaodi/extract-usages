(function(root, factory) {
    ...
    if (typeof define === 'function' && define.amd) {
        ...
        factory((root.esrefactor = {}), esprima, estraverse, esrefactor);
    }
    ...
    function cutSingleFold(folds, cuttingLine) {
        for (var i = 0; i < folds.length; i++) {
            if ((folds[i].start < cuttingLine) &&
                ...
                //replace with two
                var remaining = folds.splice(i + 1),
                    oldStart = folds[i].start,
                    oldEnd = folds[i].end;
                ...
                folds.push.apply(folds, remaining);
                break;
            }
            if ((folds[i].start === cuttingLine) &&
                (folds[i].end === cuttingLine)) {
